...
bool DiveFilter::setFilterStatus(struct dive *d, bool shown,
				 const std::vector<dive *> selection, std::vector<dive *> &removeFromSelection) const
{
...
void DiveFilter::updateDiveStatus(dive *d, bool newStatus, ShownChange &change,
				  const std::vector<dive *> selection, std::vector<dive *> &removeFromSelection) const
{
	if (setFilterStatus(d, newStatus, selection, removeFromSelection)) {
		if (newStatus)
	...
	for (dive *d: dives) {
		...
		bool newStatus = doDS        ? dive_sites.contains(d->dive_site) :
				 doFullText  ? fulltext_dive_matches(d, filterData.fullText, filterData.fulltextStringMode) && showDive(d) :
					       showDive(d);
		updateDiveStatus(d, newStatus, res, selection, removeFromSelection);
	}
